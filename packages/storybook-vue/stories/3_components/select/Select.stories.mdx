import { Meta, Story, Props, Preview, Description } from '@storybook/addon-docs/blocks';
import ScaleSelect from "./ScaleSelect.vue";
import description from './select.md'
import { action } from "@storybook/addon-actions";
export const ICON = 'M16.471,8.321 L12.613594,0.502886112 L8.755,8.321 L0.126435258,9.57533801 L6.37,15.66 L4.89610548,24.2548735 L12.613,20.197 L20.3310826,24.2548735 L18.856,15.66 L25.1007528,9.57533801 L16.471,8.321 Z M9.41891527,9.23556376 L12.613,2.761 L15.8082728,9.23556376 L22.951,10.273 L17.7826928,15.3122039 L19.003,22.427 L12.613594,19.067774 L6.223,22.427 L7.44449521,15.3122039 L2.275,10.273 L9.41891527,9.23556376 Z';

<Meta
  title='Components/Select'
  component={ScaleSelect}
  argTypes={{}}
/>

export const Template = (args, { argTypes }) => ({
  components: { ScaleSelect },
  props: {
    label: String,
    ...ScaleSelect.props
  },
  template: `
    <scale-input
      :name="name"
      :variant="variant"
      :type="type"
      :label="label"
      :helper-text="helperText"
      :status="status"
      :disabled="disabled"
      :required="required"
      :placeholder="placeholder"
      :size="size"
      :value="value"
      :input-id="inputId"
      :visible-size="visibleSize"
      @scaleChange="scaleChange"
      @scaleFocus="scaleFocus"
      @scaleBlur="scaleBlur"
      @scaleKeyDown="scaleKeyDown"
    >
      <option value="A">Category 1</option>
      <optgroup label="Category Group">
        <option value="A">Category 2</option>
      </optgroup>
      <optgroup label="Category Group 2">
        <option value="A">Category 3</option>
      </optgroup>
    </scale-input>
  `,
  methods: {
    scaleChange: action('scaleChange'),
    scaleFocus: action('scaleFocus'),
    scaleBlur: action('scaleBlur'),
    scaleKeyDown: action('scaleKeyDown'),
  }
})

<Description>{description}</Description>

<Preview withSource="none">
  <Story
    name='Standard'
    args={{
      label: 'Select',
      type: 'select',
    }}
  >
    {Template.bind({})}
  </Story>
</Preview>

<Props story='Standard' />

```html
<scale-input type="select" label="select">
  <option value="1">item 1</option>
  <option value="2" selected>item 2</option>
  <option value="3">item 3</option>
</scale-input>
```


## Small

<Preview withSource="none">
  <Story
    name='Small'
    args={{
      label: 'Small',
      type: 'select',
      size: 'small',
      variant: 'animated',
      placeholder: 'Small'
    }}
  >
    {Template.bind({})}
  </Story>
</Preview>


```html
<scale-input type="select" label="Small" size="small" variant="animated" placeholder="Small" disabled>
  <option value="1">item 1</option>
  <option value="2" selected>item 2</option>
  <option value="3">item 3</option>
</scale-input>
```


## Disabled

<Preview withSource="none">
  <Story
    name='Disabled'
    args={{
      label: 'Disabled',
      type: 'select',
      disabled: true
    }}
  >
    {Template.bind({})}
  </Story>
</Preview>


```html
<scale-input type="select" label="Disabled" disabled>
  <option value="1">item 1</option>
  <option value="2" selected>item 2</option>
  <option value="3">item 3</option>
</scale-input>
```


## Error

<Preview withSource="none">
  <Story
    name='Error'
    args={{
      label: 'Error',
      type: 'select',
      status: 'error',
      helperText: 'Something went wrong...',
    }}
  >
    {Template.bind({})}
  </Story>
</Preview>


```html
<scale-input type="select" label="Error" status="error" helper-text="Something went wrong...">
  <option value="1">item 1</option>
  <option value="2" selected>item 2</option>
  <option value="3">item 3</option>
</scale-input>
```
