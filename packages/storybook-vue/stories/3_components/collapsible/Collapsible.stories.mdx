...
export const Template = (args, { argTypes }) => ({
  ...
  template: `
    ...
    @scaleKeyDown="scaleKeyDown">
      <template #headline><scale-text slot="headline" variant="collapsible">{{ headline }}</scale-text></template>
      <template #content><p slot="content">{{ content }}</p></template>
    </scale-collapsible>
  `,
  methods: {
...
```html
<scale-collapsible>
  <foo slot="headline">This is the headline</foo>
  <bar slot="content">This is the content</bar>
</scale-collapsible>
```
