...
Before you install any packages, make sure you are logged into your npm account locally. You will also need to be part of the scaleds organization on npm. (If you need an invitation please send an email to Egor egor.kirpichev@telekom.de.)
...
RRegardless of what CI platform / tech / service you use, the system must be able to access the @scaleds/\* packages used in your application.

Therefore, you will need the following three things:
...
This file is used to easily constrain a set of npm-related configurations to a project. In this case, we use it to set up the authorization token so the system can access the packages without requiring a particular user to be logged in.
...
With this setup the system will be able to login itself to npm and gain access to the packages. In case you need any help, do not hesitate to contact us.
