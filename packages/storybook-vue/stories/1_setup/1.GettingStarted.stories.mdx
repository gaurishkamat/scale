...
## Package publishing (releases)

Currently there's no CI in place to manage publishing packages to the `npm` registry. We need to do it manually.

> Heads-up: You need either `maintainer` or `owner` rights in the GitLab repository in order to push directly to `master`.

Make sure all tests pass and there are no formatting/linting errors.

### Step 1

- In the `master` branch, run `yarn new-version` at the root of the project. [Lerna](https://github.com/lerna/lerna/tree/master/commands/version#lernaversion) will take care of automatically bumping versions for all packages.
- Push.

### Step 2

Build (`yarn build`) and publish (`npm publish`), in that order, all packages individually:
