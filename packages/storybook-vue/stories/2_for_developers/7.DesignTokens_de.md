...
Ein zentraler Bestandteil des Scale Design System sind die Design Token.

Scale Komponenten nutzen die Token um Konsistenz zu erzeugen.

Die Token sind unabhängig von den Komponenten und liegen in CSS und Javascript Formaten vor. So kannst Du die Design Token von Scale auch in deinen eigenen Komponenten verwenden.

## Mit token arbeiten

Tokens sind als CSS Variablen mit dem Präfix `scl` verfügbar.

Zusätzlich sind sie als benamter export für die Nutzung in Javascript verfügbar (CSS-in-JS).

Um die Token zu nutzen, importiere Sie aus dem Package `@scaleds/design-tokens-telekom`.
...
Der Quellcode ist im Ordner `src/`  verfügbar.
