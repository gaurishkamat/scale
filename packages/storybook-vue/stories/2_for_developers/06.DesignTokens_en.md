...
The heart of the Scale design system is a set of design tokens. They store variables such as typography, colors, spacing, or roundings.
This creates a scalable and visually consistent system.
...
Tokens are available as CSS variables, with the prefix with `scl`.

They're also available as named exports to use in JavaScript (CSS-in-JS).

To use the Telekom design tokens, import them from the `@scaleds/telekom-design-tokens` package

- `@scaleds/telekom-design-tokens/dist/design-tokens-telekom.css` for **CSS**
- `@scaleds/telekom-design-tokens/dist/design-tokens-telekom.js` for **JavaScript** (or CSS-in-JS)

The source is also available in the src/ folder.

### Font formatting

To style the text in your project, there are a few CSS helper classes:
