...
import { r as registerInstance, h, H as Host, g as getElement, c as createEvent } from './index-c2e5d8cb.js';
import { c as createCommonjsModule, g as getTheme, a as combineObjects, d as defaultTheme } from './theme-6300d255.js';
...
function createRule(name, decl, options) {
    ...
    if (rule)
        return rule; // It is an at-rule and it has no instance.
    return null;
}
...
function toCssValue(value, ignoreImportant) {
    ...
    if (Array.isArray(value[0])) {
        ...
        cssValue = join(value, ', '); // Add !important, because it was ignored.
...
function toCss(selector, style, options) {
    ...
    if (!result && !options.allowEmpty)
        return result; // When rule is being stringified before selector was defined.
    if (!selector)
        return result;
    indent--;
    if (result)
...
function () {
    ...
    _proto.prop = function prop(name, value, options) {
        // It's a getter.
        if (value === undefined)
            return this.style[name]; // Don't do anything if the value has not changed.
        ...
        if (isEmpty && !isDefined && !force)
            return this; // We are going to remove this value.
        var remove = isEmpty && isDefined;
        if (remove)
            delete this.style[name];
        else
            this.style[name] = newValue; // Renderable is defined if StyleSheet option `link` is true.
...
function () {
    ...
    _proto.addRule = function addRule(name, decl, options) {
        ...
        if (this.attached) {
            if (!this.deployed)
                return rule; // Don't insert rule directly if there is no stringified version yet.
...
if (globalThis[ns] == null)
    globalThis[ns] = 0; // Bundle may contain multiple JSS versions at the same time. In order to identify
    ...
    return {
        onCreateRule: function onCreateRule(name, decl, options) {
            if (!isObservable(decl))
                return null; // Cast `decl` to `Observable`, since it passed the type guard.
            var style$ = decl;
...
function jssExtend() {
    ...
    function onChangeValue(value, prop, rule) {
        if (prop !== 'extend')
            return value; // Value is empty, remove properties set previously.
    ...
    function replaceParentRefs(nestedProp, parentProp) {
        ...
        for (var i = 0; i < parentSelectors.length; i++) {
            ...
            for (var j = 0; j < nestedSelectors.length; j++) {
                ...
                if (result)
                    result += ', '; // Replace all & by the parent or prefix & with the parent.
    ...
    function onProcessStyle(style, rule, sheet) {
        ...
        for (var prop in style) {
            ...
            if (isNested) {
                ...
                if (!replaceRef)
                    replaceRef = getReplaceRef(container, sheet); // Replace all $refs.
...
function registerClass(rule, className) {
    // Skip falsy values
    if (!className)
        return true; // Support array of class names `{composes: ['foo', 'bar']}`
...
function supportedKeyframes(key) {
    ...
    if (key[1] === '-')
        return key; // No need to prefix IE/Edge. Older browsers will ignore unsupported rules.
...
var prefixed = {
    supportedProperty: function supportedProperty(prop, style) {
        ...
        if (prop[0] === '-')
            return prop; // Return already prefixed value without prefixing.
        if (prop[0] === '-' && prop[1] === '-')
            return prop;
        if (prefix.js + pascalized in style)
            return prefix.css + prop; // Try webkit fallback.
...
function supportedProperty(prop, options) {
    ...
    if (!el)
        return prop; // Remove cache for benchmark tests or return property from the cache.
...
function supportedValue(property, value) {
    ...
    if (!el$1 || property === 'content')
        return value; // It is a string or a number as a string like '1'.
...
var ScaleSheetManager = /** @class */ (function () {
    function ScaleSheetManager() {
        this.sheetManager = new SheetsManager();
        this.keyMap = new Map();
    }
    ScaleSheetManager.prototype._get = function (key) {
        var keyObj = this.keyMap.get(key);
        return this.sheetManager.get(keyObj);
    };
    ScaleSheetManager.prototype._set = function (key, sheet) {
        var keyObj = { key: key };
        this.keyMap.set(key, keyObj);
        this.sheetManager.add(keyObj, sheet);
    };
    ScaleSheetManager.prototype._manage = function (key) {
        this.sheetManager.manage(this.keyMap.get(key));
    };
    ScaleSheetManager.prototype.unmanage = function (key) {
        this.sheetManager.unmanage(this.keyMap.get(key));
    };
    ScaleSheetManager.prototype.load = function (prevKey, nextKey, injectedValues) {
        if (prevKey && prevKey !== nextKey) {
            this.unmanage(prevKey);
        }
        var oldSheet = this._get(nextKey);
        var newSheet = !oldSheet && index.createStyleSheet(injectedValues, { link: true });
        if (newSheet) {
    ...
    return ScaleSheetManager;
}());
var manager = new ScaleSheetManager();
index.setup(index$1());
var getKeys = function (obj) {
    var whitelistedProps = ['styles'];
    var keys = [];
    for (var key_1 in obj) {
        if (obj[key_1] && !whitelistedProps.includes(key_1)) {
            keys.push(key_1);
        }
    }
    return keys;
};
var getComponentKey = function (componentKey, that) {
    return getKeys(that).reduce(function (acc, keyName) {
        var _a;
        try {
            return acc + "-" + JSON.stringify((_a = {}, _a[keyName] = that[keyName], _a));
        }
        catch (err) {
            return acc;
        }
...
function CssInJs(componentKey, styles) {
    var withInjectedValues = function (that) {
        var withDefaultTheme;
        var stylesAttributeValue;
        try {
            withDefaultTheme = combineObjects(styles, getTheme().components[componentKey]);
        }
        catch (error) {
        ...
        var combined = that.styles
            ? combineObjects(withDefaultTheme, stylesAttributeValue)
            : withDefaultTheme;
        return combined;
    };
    return function (target, propertyKey) {
        var render = target.render, componentDidUnload = target.componentDidUnload;
        target.render = function () {
            var newKey = getComponentKey(componentKey, this);
            if (this.key !== newKey) {
...
var Alert = /** @class */ (function () {
    function class_1(hostRef) {
        ...
        this.onCloseAlertWithTimeout = function () {
            if (_this_1.timeout !== false) {
                if (typeof _this_1.timeout === 'number') {
                    setTimeout(_this_1.close, _this_1.timeout);
                }
                else {
                    setTimeout(_this_1.close, _this_1.defaultTimeout);
                }
            }
            else {
                return null;
    ...
    class_1.prototype.render = function () {
        ...
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h("div", { class: this.getCssClassMap() }, h("div", { class: classes.alert__body }, h("div", { class: classes.alert__icon }, this.icon), h("div", { class: classes.alert__content }, h("div", { class: classes.alert__headline }, this.headline), h("slot", null))), h("a", { class: classes.alert__close, onClick: this.close }, this.hasSlotClose ? (h("div", { class: classes['alert__close-icon'] }, h("slot", { name: "close" }))) : ('x')))));
    };
    class_1.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
...
var Breadcrumb = /** @class */ (function () {
    ...
    Breadcrumb.prototype.componentWillLoad = function () {
        if (this.linksArray.length === 0) {
            this.setLinksArray();
        }
        this.separatorSlot = this.hostElement.querySelector('[slot="separator"]');
    };
    Breadcrumb.prototype.componentWillUpdate = function () { };
    Breadcrumb.prototype.componentDidUnload = function () { };
    Breadcrumb.prototype.setLinksArray = function () {
        this.linksArray = Array.from(this.hostElement.children).filter(function (element) { return element.slot === ''; });
    };
    Breadcrumb.prototype.render = function () {
        var _this_1 = this;
        var classes = this.stylesheet.classes;
        var isLast = function (index) { return index === _this_1.linksArray.length - 1; };
        // Set aria-current="page" to the last item if it's a link
        var getCurrentAttr = function (index) { return isLast(index) === true ? { 'aria-current': 'page' } : undefined; };
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h("nav", { "aria-label": "Breadcrumb", class: this.getCssClassMap() }, h("ol", null, this.linksArray.map(function (element, index) {
            var separator = _this_1.separatorSlot != null ? (h("span", { class: classes.separator, innerHTML: _this_1.separatorSlot.innerHTML })) : (h("span", { class: classes.separator }, _this_1.separator));
            return (h("li", null, element.href ? (h("a", Object.assign({ href: element.href, class: classnames(isLast(index) && classes.current, classes.link) }, getCurrentAttr(index)), element.textContent)) : (h("span", { class: classnames(isLast(index) && classes.current, classes.item) }, element.textContent)), !isLast(index) ? separator : null));
        })))));
    };
    Breadcrumb.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
        return classnames(classes.breadcrumb);
    };
    Object.defineProperty(Breadcrumb.prototype, "hostElement", {
        get: function () { return getElement(this); },
        enumerable: true,
        configurable: true
    });
...
var getTransition = function (_a, selectors, duration, easing) {
    var transitions = _a.transitions;
    return selectors + " " + transitions.duration[duration] / 1000 + "s " + transitions.easing[easing];
};
var buttonVariant = function (variant) {
    var _a;
    return (_a = {},
        _a["button--variant-" + variant] = {
            color: function (_a) {
                var colors = _a.colors;
                return colors[variant].contrastText;
            },
            '--icon-color': function (_a) {
                var colors = _a.colors;
                return colors[variant].contrastText;
            },
            ...
            border: function (_a) {
                var colors = _a.colors;
                return "1px solid " + colors[variant].default;
            },
            '&:hover': {
                color: function (_a) {
                    var colors = _a.colors;
                ...
                background: function (_a) {
                    var colors = _a.colors;
                    return colors[variant].darker;
                },
                border: function (_a) {
                    var colors = _a.colors;
                    return "1px solid " + colors[variant].darker;
                },
            },
        },
        _a);
};
var styles$2 = Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({ button: {
        color: function (_a) {
            var colors = _a.colors;
        ...
        fontFamily: function (_a) {
            var typography = _a.typography;
            return typography.variants.button.fontFamily;
        },
        fontSize: function (_a) {
            var typography = _a.typography;
            return typography.variants.button.fontSize;
        },
        fontWeight: function (_a) {
            var typography = _a.typography;
            return typography.variants.button.fontWeightRegular;
        },
        textTransform: function (_a) {
            var typography = _a.typography;
            return typography.variants.button.textTransform;
        },
        letterSpacing: function (_a) {
            var typography = _a.typography;
            return typography.variants.button.letterSpacing;
        },
        transition: function (theme) { return getTransition(theme, 'all', 'shorter', 'easeInOut'); },
        boxSizing: 'border-box',
        position: 'relative',
        display: 'inline-flex',
        justifyContent: 'center',
        ...
        '&:hover': {
            ...
            transition: function (theme) { return getTransition(theme, 'all', 'standard', 'easeInOut'); },
            boxShadow: 'none',
            textDecoration: 'none',
        },
    }, 'button--disabled': {
        '&, &:hover': {
            opacity: function (_a) {
                var colors = _a.colors;
                return colors.action.disabledOpacity;
            },
            border: function (_a) {
                var colors = _a.colors;
                return "1px solid " + colors.action.disabled;
            },
            color: function (_a) {
                var colors = _a.colors;
...
var Button = /** @class */ (function () {
    ...
    class_2.prototype.render = function () {
        var classes = this.stylesheet.classes;
        var Tag = this.href ? 'a' : 'button';
        var role = this.href
            ? { role: this.role || 'button' }
            : this.role
                ? { role: this.role }
                : {};
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h(Tag, Object.assign({ class: this.getCssClassMap(), tabindex: this.focusable ? 0 : -1 }, (!!this.href ? { href: this.href } : {}), (!!this.href ? { target: this.target } : {}), (!!!this.href ? { disabled: this.disabled } : {}), (!!this.ariaLabel ? { 'aria-label': this.ariaLabel } : {}), role), !!this.icon === false &&
            (!!this.iconBefore === true || this.hasSlotBefore) && (h("div", { class: classes.button__before }, !!this.iconBefore ? (h("scale-icon", { path: this.iconBefore, size: this.iconSize })) : (h("slot", { name: "before" })))), this.icon && this.icon !== '' ? (h("scale-icon", { path: this.icon, size: this.iconSize })) : (h("slot", null)), !!this.icon === false &&
    ...
    class_2.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
    ...
    Object.defineProperty(class_2.prototype, "hostElement", {
        get: function () { return getElement(this); },
        enumerable: true,
        configurable: true
    });
...
var Card = /** @class */ (function () {
    ...
    Card.prototype.componentWillUpdate = function () { };
    Card.prototype.componentDidUnload = function () { };
    Card.prototype.render = function () {
        var classes = this.stylesheet.classes;
        var Tag = !!this.href ? 'a' : 'div';
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h(Tag, Object.assign({ class: this.getCssClassMap() }, (!!this.href ? { href: this.href } : {}), (!!this.href ? { target: this.target } : {}), (!!this.interactive ? { tabindex: 1 } : {})), h("div", { class: classes.card__body }, h("slot", null)))));
    };
...
var Carousel = /** @class */ (function () {
    function Carousel(hostRef) {
        ...
        this.slidesArray = [];
        this.value = 0;
        this.handleSlideChange = function (direction) {
            var val = _this_1.value;
            if (direction === 'prev') {
                val === 0
                    ? (_this_1.value = -100 * (_this_1.slidesArray.length - 1))
                    : (_this_1.value = val + 100);
            }
            if (direction === 'next') {
                val === -100 * (_this_1.slidesArray.length - 1)
                    ? (_this_1.value = 0)
                    : (_this_1.value = val - 100);
            }
        };
        this.setActiveSlide = function (index) {
            _this_1.value = -100 * index;
        };
        this.setTransformValue = function () {
            if (!!_this_1.vertical) {
                return "translateY(" + _this_1.value + "%)";
            }
            return "translateX(" + _this_1.value + "%)";
        };
        this.setActiveCssClass = function (index) {
            if (Math.abs(_this_1.value) / 100 === index) {
        ...
        if (this.slidesArray.length === 0) {
            ...
            for (var childIndex = 0; childIndex < children.length; childIndex++) {
                if (children[childIndex].slot === '') {
                    // tslint:disable-next-line: prefer-for-of
                    for (var slideIndex = 0; slideIndex < children[childIndex].children.length; slideIndex++) {
                        var element = children[childIndex].children[slideIndex];
    ...
    Carousel.prototype.componentWillUpdate = function () { };
    Carousel.prototype.componentDidUnload = function () { };
    Carousel.prototype.render = function () {
        var _this_1 = this;
        var classes = this.stylesheet.classes;
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h("div", { class: this.getCssClassMap() }, h("div", { class: classes.carousel__container }, h("div", { class: classes.carousel__arrow + " " + classes['carousel__arrow--left'], onClick: function () { return _this_1.handleSlideChange('prev'); } }, h("slot", { name: "arrow-left" })), this.slidesArray.map(function (element) { return (h("div", { class: classes.carousel__slide, style: { transform: _this_1.setTransformValue() } }, h("div", { innerHTML: element.outerHTML }))); }), h("div", { class: classes.carousel__arrow + " " + classes['carousel__arrow--right'], onClick: function () { return _this_1.handleSlideChange('next'); } }, h("slot", { name: "arrow-right" }))), h("ul", { class: classes.carousel__indicators }, Array.from(Array(this.slidesArray.length).keys()).map(function (index) { return (h("li", { key: index, class: classes.carousel__indicator + " " + _this_1.setActiveCssClass(index), onClick: function () { return _this_1.setActiveSlide(index); } })); })))));
    };
    Carousel.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
        return classnames(classes.carousel, this.customClass && this.customClass, this.vertical && classes['carousel--vertical']);
    };
    Object.defineProperty(Carousel.prototype, "hostElement", {
        get: function () { return getElement(this); },
        enumerable: true,
        configurable: true
    });
...
var Divider = /** @class */ (function () {
    ...
    Divider.prototype.componentWillUpdate = function () { };
    Divider.prototype.componentDidUnload = function () { };
    Divider.prototype.render = function () {
        var classes = this.stylesheet.classes;
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h("div", { class: this.getCssClassMap() }, !this.vertical ? h("hr", null) : h("span", { class: classes.divider__vertical }))));
    };
    Divider.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
...
var Icon = /** @class */ (function () {
    ...
    Icon.prototype.componentWillUpdate = function () { };
    Icon.prototype.componentDidUnload = function () { };
    Icon.prototype.render = function () {
        var hostStyles = "\n:host {\n  height: auto;\n  width: auto;\n  display: inline-flex;\n  justify-content: center;\n  align-items: center;\n}\npath {\n  transition: all .2s ease-in-out;\n}\n";
        return (h(Host, null, h("style", null, hostStyles, this.stylesheet.toString()), h("svg", Object.assign({}, (this.focusable ? { tabindex: 0 } : {}), { class: this.getCssClassMap(), width: this.size || this.width, height: this.size || this.height, viewBox: "0 0 26 26" }), h("path", { d: this.path, stroke: "transparent", fill: "transparent" }))));
    };
    Icon.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
...
var defaultTransition = 'all 0.2s ease-in-out';
var animated = function (size) { return ({
    start: {
        fontSize: 16,
        color: '#dfdfdf',
...
var styles$7 = {
    ...
    'input--status-error': {
        '&.animated .input__label, & label': {
            color: function (_a) {
                var colors = _a.colors;
                return colors.error.default + " !important";
            },
        },
        ...
        '& .input__input': {
            border: function (_a) {
                var colors = _a.colors;
                return "1px solid " + colors.error.default;
            },
        },
        '& .input__helper-text': {
            color: function (_a) {
    ...
    'input--type-radio': {
        ...
        '& .input__radio': {
            ...
            '&:checked, &:focus': {
                border: function (_a) {
                    var colors = _a.colors;
                    return "6px solid " + colors.primary.default;
                },
                transition: defaultTransition,
                outline: 'none',
            },
...
var Input = /** @class */ (function () {
    ...
    Input.prototype.componentWillUpdate = function () { };
    ...
    Input.prototype.render = function () {
        ...
        if (this.type === 'checkbox') {
            return (h(Host, null, h("style", null, this.stylesheet.toString()), h("div", { class: this.getCssClassMap() }, h("div", { class: classnames('input__checkbox-container') }, h("input", { type: "checkbox", name: this.name, class: classnames('input__checkbox'), id: this.inputId, onChange: function (event) { return _this_1.handleChange(event); }, value: this.value, checked: this.checked, disabled: this.disabled }), h("span", { class: classnames('input__checkbox-placeholder'), tabIndex: 1 }), !!this.checked && !!this.icon && (h("scale-icon", { path: this.icon }))), h("label", { class: "input__label", htmlFor: this.name }, this.label))));
        }
        if (this.type === 'radio') {
            return (h(Host, null, h("style", null, this.stylesheet.toString()), h("div", { class: this.getCssClassMap() }, h("input", { type: "radio", name: this.name, class: classnames('input__radio'), id: this.inputId, onChange: function (event) { return _this_1.handleChange(event); }, value: this.value, checked: this.preChecked, disabled: this.disabled }), h("label", { class: "input__label", htmlFor: this.inputId }, this.label))));
        }
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h("div", { class: this.getCssClassMap() }, !!this.label && this.variant === 'static' && (h("label", { class: "input__label" }, this.label)), h("input", { type: this.type, class: classnames('input__input', this.label && 'has-label'), value: this.value, name: this.name, required: this.required, minLength: this.minLength, maxLength: this.maxLength, onInput: function (event) { return _this_1.handleChange(event); }, onFocus: function (event) { return _this_1.handleFocus(event); }, onBlur: function (event) { return _this_1.handleBlur(event); }, onKeyDown: function (event) { return _this_1.handleKeyDown(event); }, placeholder: this.placeholder, disabled: this.disabled }), !!this.label && this.variant === 'animated' && (h("label", { class: "input__label" }, this.label)), (!!this.helperText || !!this.counter) && (h("div", { class: "input__meta" }, !!this.helperText && (h("div", { class: "input__helper-text" }, this.helperText)), this.counter && (h("div", { class: "input__counter" }, !!this.value ? this.value.length : 0, " / ", this.maxLength)))))));
    };
    Input.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
        var isAnimated = (!!this.placeholder || !!this.value) &&
            !!this.label &&
...
var Link = /** @class */ (function () {
    ...
    Link.prototype.componentWillUpdate = function () { };
    Link.prototype.componentDidUnload = function () { };
    Link.prototype.render = function () {
        var Tag = !this.disabled ? 'a' : 'div';
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h(Tag, Object.assign({ class: this.getCssClassMap() }, (!this.disabled ? { href: this.href } : {}), (!this.disabled ? { target: this.target } : {})), h("slot", null), this.icon && this.icon !== '' && (h("scale-icon", { path: this.icon, size: this.iconSize })))));
    };
    Link.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
...
var List = /** @class */ (function () {
    ...
    List.prototype.componentWillUpdate = function () { };
    List.prototype.componentDidUnload = function () { };
    List.prototype.render = function () {
        ...
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h(Tag, { class: this.getCssClassMap() }, h("slot", null))));
    };
    List.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
...
var ListItem = /** @class */ (function () {
    ...
    ListItem.prototype.componentDidUnload = function () { };
    ListItem.prototype.componentWillUpdate = function () { };
    ListItem.prototype.render = function () {
        this.stylesheet.update({
            type: this.type,
        });
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h("li", { class: this.getCssClassMap() }, !!this.icon && (h("scale-icon", { path: this.icon, size: this.iconSize })), h("slot", null))));
    };
    ListItem.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
...
var Modal = /** @class */ (function () {
    ...
    class_3.prototype.render = function () {
        ...
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h("div", { class: this.getCssClassMap() }, h("div", { class: classes.modal__backdrop, onClick: this.closeModal }), h("div", { class: classes.modal__content }, this.hasSlotHeader /* istanbul ignore next */ && (h("div", { class: classes.modal__header }, h("slot", { name: "header" }), h("a", { class: classes.modal__close, onClick: this.closeModal }, this.hasSlotClose ? (h("div", { class: classes['modal__close-icon'] }, h("slot", { name: "close" }))) : ('x')))), h("div", { class: classes.modal__body }, h("slot", null)), this.hasSlotActions /* istanbul ignore next */ && (h("div", { class: classes.modal__actions }, h("slot", { name: "modal-actions" })))))));
    };
    class_3.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
    ...
    Object.defineProperty(class_3.prototype, "hostElement", {
        get: function () { return getElement(this); },
        enumerable: true,
        configurable: true
    });
...
var ProgressBar = /** @class */ (function () {
    ...
    ProgressBar.prototype.componentWillUpdate = function () { };
    ProgressBar.prototype.componentDidUnload = function () { };
    ProgressBar.prototype.render = function () {
        var classes = this.stylesheet.classes;
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h("div", { class: classes['progress-bar'] }, h("div", { class: classes['progress-bar__outer'], style: { height: this.strokeWidth + "px" } }, h("div", { class: classes['progress-bar__inner'] + " " + this.getCssClassMap(), style: { width: this.percentage + "%" } }, !!this.textInside && (h("div", { class: classes['progress-bar__inner-text'] }, this.percentage + "%")))), !!this.showText && (h("div", { class: classes['progress-bar__text'] }, this.percentage + "%")))));
    };
    ProgressBar.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
...
var Slider = /** @class */ (function () {
    function Slider(hostRef) {
        ...
        this.updateValue = function (event) {
            _this_1.value = event.target.value;
        };
    }
    ...
    Slider.prototype.componentWillUpdate = function () { };
    Slider.prototype.componentDidUnload = function () { };
    Slider.prototype.render = function () {
        var classes = this.stylesheet.classes;
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h("div", { class: this.getCssClassMap() }, h("input", { class: classes.slider__input, type: "range", min: this.min, max: this.max, value: this.value, step: this.step, onInput: this.updateValue }), this.label && h("span", { class: classes.slider__text }, this.value))));
    };
    Slider.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
...
var Switch = /** @class */ (function () {
    function Switch(hostRef) {
        ...
        this.toggleSwitch = function () {
            ...
            _this_1.active = !_this_1.active;
        };
    }
    Switch.prototype.componentWillUpdate = function () { };
    Switch.prototype.componentDidUnload = function () { };
    Switch.prototype.render = function () {
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h("div", { class: this.getCssClassMap(), onClick: this.toggleSwitch })));
    };
    Switch.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
...
var Table = /** @class */ (function () {
    ...
    Table.prototype.componentWillLoad = function () {
        this.hostElement.querySelectorAll('th').forEach(function (th) {
    ...
    Table.prototype.componentDidUnload = function () { };
    Table.prototype.render = function () {
        return (h(Host, { class: this.getCssClassMap() }, h("style", null, this.stylesheet.toString()), h("slot", null)));
    };
    Table.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
    ...
    Object.defineProperty(Table.prototype, "hostElement", {
        get: function () { return getElement(this); },
        enumerable: true,
        configurable: true
    });
...
var tagVariant = function (variant) {
    var _a;
    return (_a = {},
        _a["tag--variant-" + variant] = {
            color: function (_a) {
                var colors = _a.colors;
                return colors[variant].contrastText;
            },
...
var tagVariantLink = function (variant) {
    var _a;
    return (_a = {},
        _a["&$tag--variant-" + variant] = Object.assign(Object.assign({}, tagVariant(variant)), { '&:hover': {
                color: function (_a) {
                    var colors = _a.colors;
                    return colors[variant].contrastText;
                },
...
var styles$g = Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({ tag: {
        display: 'inline-flex',
        alignItems: 'center',
        justifyContent: 'center',
        padding: '6px 12px',
...
var Tag = /** @class */ (function () {
    ...
    Tag.prototype.componentDidUnload = function () { };
    Tag.prototype.handleClose = function (event) {
    ...
    Tag.prototype.render = function () {
        var _this_1 = this;
        var Element = !!this.href && !this.disabled ? 'a' : 'span';
        var linkProps = !!this.href
            ? {
                href: this.href,
                target: this.target,
            }
            : {};
        var iconProps = !this.disabled
            ? {
                focusable: true,
                onClick: function (event) { return _this_1.handleClose(event); },
            }
            : {};
        var theme = typeof window !== 'undefined' &&
            window.scale &&
            window.scale.theme;
        var icons = (theme || defaultTheme).icons;
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h(Element, Object.assign({ class: this.getCssClassMap() }, linkProps), h("slot", null), this.dismissable && (h("scale-icon", Object.assign({ size: 16, path: icons.close }, iconProps))))));
    };
    Tag.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
...
var variants = function () {
    var variantClasses = {};
    var themeVariants = getTheme().typography.variants;
    Object.keys(themeVariants).map(function (variant) {
...
var Text = /** @class */ (function () {
    ...
    Text.prototype.componentWillUpdate = function () { };
    Text.prototype.componentDidUnload = function () { };
...
var Toast = /** @class */ (function () {
    function class_4(hostRef) {
        ...
        this.timerId = null;
        this.close = function () {
            clearInterval(_this_1.timerId);
            _this_1.hideToast = true;
            setTimeout(function () {
                _this_1.timerId = null;
                _this_1.opened = false;
                _this_1.progress = 0;
            }, _this_1.fadeDuration);
        };
        this.getTime = function () {
            var formattedTime = _this_1.time &&
                formatDistance$1(subSeconds(_this_1.time, 3), new Date(), { addSuffix: true });
            return formattedTime;
        };
        this.setToastTimeout = function () {
            if (_this_1.opened && _this_1.autoHide !== false && !_this_1.timerId) {
                _this_1.timerId = setInterval(function () {
                    _this_1.progress += 1 / (_this_1.getAutoHide() / 1000);
                    if (_this_1.progress >= 100) {
                        _this_1.close();
                    }
                }, 10);
            }
        };
        this.transitions = function (offset) { return "\n    @keyframes fadeIn {\n      from {\n        opacity: 0;\n        top: -" + offset + "px;\n      }\n      to {\n        opacity: 1;\n        top: " + _this_1.positionTop + "px;\n      }\n    }\n\n    @keyframes fadeOut {\n      from {\n        opacity: 1;\n        top: " + _this_1.positionTop + "px;\n      }\n      to {\n        opacity: 0;\n        top: -" + offset + "px;\n      }\n    }\n  "; };
        this.animationStyle = function (offset) {
            _this_1.stylesheet.addRule('toast--show', {
                right: _this_1.positionRight + "px",
                animation: "fadeIn " + _this_1.fadeDuration / 1000 + "s ease-in-out",
                top: _this_1.positionTop + "px",
                opacity: 1,
            });
            _this_1.stylesheet.addRule('toast--hide', {
                right: _this_1.positionRight + "px",
                animation: "fadeOut " + _this_1.fadeDuration / 1000 + "s ease-in-out",
                top: "-" + offset + "px",
                opacity: 0,
            });
        };
    ...
    class_4.prototype.componentWillUpdate = function () { };
    /** Toast method: open() */
    ...
    class_4.prototype.render = function () {
        ...
        return (h(Host, null, h("style", null, this.stylesheet.toString()), h("style", null, this.transitions(this.toastHeightWithOffset)), h("style", null, this.animationStyle(this.toastHeightWithOffset)), h("div", { class: this.getCssClassMap() }, h("div", { class: classes.toast__header }, h("slot", { name: "header" }), "header", h("small", null, this.getTime()), h("a", { onClick: this.close }, h("span", { "aria-hidden": "true" }, "\u00D7"))), this.autoHide && (h("div", { class: classes.toast__progress, style: { width: this.progress + "%" } }, "\u00A0")), h("div", { class: classes.toast__body }, h("slot", null)))));
    };
    class_4.prototype.getToastHeightWithOffset = function () {
        var classes = this.stylesheet.classes;
        var toastHeight = this.element.shadowRoot.querySelector("." + classes.toast).scrollHeight;
        this.toastHeightWithOffset = toastHeight + this.positionTop;
    };
    class_4.prototype.getAutoHide = function () {
    ...
    class_4.prototype.getCssClassMap = function () {
        var classes = this.stylesheet.classes;
...
export { Alert as scale_alert, Breadcrumb as scale_breadcrumb, Button as scale_button, Card as scale_card, Carousel as scale_carousel, Divider as scale_divider, Icon as scale_icon, Input as scale_input, Link as scale_link, List as scale_list, ListItem as scale_list_item, Modal as scale_modal, ProgressBar as scale_progress_bar, Slider as scale_slider, Switch as scale_switch, Table as scale_table, Tag as scale_tag, Text as scale_text, Toast as scale_toast };
