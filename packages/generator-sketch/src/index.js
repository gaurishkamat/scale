...
(async function() {
  ...
  function makeStyleKey(style) {
    const borders = (style.borders || []).filter(b => b.color.alpha > 0).map(border => {
      return [
    ...
    return `borders:${borders.sort().join("|")};fills:${fills.sort().join("|")}`;
  }
  ...
  function colorString(fillOrBorder) {
    ...
    return `#${hex(Math.floor(red*255.999))}${hex(Math.floor(green*255.999))}${hex(Math.floor(blue*255.999))}${alpha < 1 ? ' opacity ' + Math.floor(alpha*1000)/1000 : ''}`;
  }

  function makeStyleName(override) {
    var styleName = "";
    if (override.borders) override.borders = override.borders.filter(b => b.color.alpha > 0);
    if (override.fills) override.fills = override.fills.filter(b => b.color.alpha > 0);
    ...
    if (override.borders && override.borders.length > 0) {
      ...
      styleName += "Border " + override.borders.map(b => `${Math.floor(100*b.thickness)/100}px ${positionString(b.position)} ${colorString(b)}` ).join(", ");
    ...
    */
    if (enhanced['_class'] === 'group' && enhanced.isSymbol === true) {
      ...
      if (isIcon) {
        ...
        iconName = enhanced.name.replace(/^X \/ icon-([^-]+)-(([^-]+-?)+)$/, 
          (_, category, name) => 
          `Icon / ${
            category
            .replace(/-|_/g, ' ')
  ...
  const designTokens = await import('@telekom/scale-design-tokens/dist/design-tokens-telekom.js');
  parseColors(sketchColors, [], designTokens.color);
