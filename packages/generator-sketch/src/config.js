...
module.exports = {
  ...
  setSymbolResizing: function(symbol) {
    ...
    if (!/^(Icon|(X \/ icon))/.test(symbol.name)) {
      ...
      findLayers(symbol, "svg.icon", icon => icon.resizingConstraint = FILL_SPACE);
    ...
    try {
      ...
      if (/^(Card)/.test(symbol.name)) {
        ...
        findLayer(symbol, 'slot', slot => {
          ...
          findLayer(slot, 'img', l => {
            findLayer(l, l => {l.resizingConstraint = FILL_SPACE});
      ...
      if (/^(Dropdown)/.test(symbol.name)) {
        ...
        findLayer(symbol, "Dropdown Label", label => {
          ...
          label.textBehaviour = 2;
          findLayer(symbol, "label.input__label", label => label.frame.width += addWidth);
        });
        ...
        findLayer(symbol, /^Information/, info => {
          ...
          findLayer(symbol, "div.input__helper-text", label => label.frame.width += addWidth);
        });
      }
      ...
      if (/^Progress Bar/.test(symbol.name)) {
        ...
        findLayer(symbol, 'Description', l => l.resizingConstraint = TOP_LEFT_FIXED_SIZE);
        findLayer(symbol, 'Progress Bar Label', l => l.resizingConstraint = TOP_LEFT_FIXED_SIZE);
        ...
        findLayer(symbol, 'div.progress-bar-wrapper', wrapper => {
          ...
          findLayer(wrapper, /^div#progress-bar-/, bar => {
            ...
            findLayer(bar, s => { s.resizingConstraint = TOP_LEFT_RIGHT_FIXED_HEIGHT; return false; });
            findLayer(bar, 'div.progress-bar__inner', l => l.resizingConstraint = TOP_LEFT_FIXED_HEIGHT);
          });
          findLayer(wrapper, s => s.name === 'Icon', icon => icon.resizingConstraint = TOP_RIGHT_FIXED_SIZE);
        });
      }
      if (/^Sidebar Nav/.test(symbol.name)) {
        ...
        if (!/Example/.test(symbol.name)) {
          ...
          findLayer(symbol, "Icon", s => s.resizingConstraint = TOP_RIGHT_FIXED_SIZE);
          findLayers(symbol, /sidebar-nav-collapsible/, s => s.resizingConstraint = TOP_LEFT_RIGHT_FIXED_HEIGHT);
          findLayers(symbol, "Background", s => s.resizingConstraint = TOP_LEFT_RIGHT_FIXED_HEIGHT);
          findLayer(symbol, "border-bottom", s => s.resizingConstraint = BOTTOM_LEFT_RIGHT_FIXED_HEIGHT)

          // Third level
          findLayer(symbol, /^(li\.sidebar-nav-item|a)$/, s => s.resizingConstraint = TOP_LEFT_RIGHT_FIXED_HEIGHT);
          findLayer(symbol, "span", s => s.resizingConstraint = TOP_LEFT_FIXED_SIZE);
          if (/Second Level .* Expanded/.test(symbol.name)) {
            findLayer(symbol, "li.sidebar-nav-collapsible", s => s.resizingConstraint = NONE);
          }
        }
      }
      ...
      if (/^Switch/.test(symbol.name)) {
        symbol.groupLayout = undefined;
        symbol.layers[0].resizingConstraint = TOP_LEFT_FIXED_SIZE;
        findLayer(symbol, /switch[-_]+wrapper/, l => l.resizingConstraint = TOP_LEFT_FIXED_SIZE);
        findLayer(symbol, 'Switch Label', l => l.resizingConstraint = FILL_SPACE);
      }
      if (/^Table/.test(symbol.name)) {
        symbol.groupLayout = undefined;
        symbol.layers.forEach(l => l.resizingConstraint = TOP_LEFT_FIXED_SIZE);
      }
      if (/^(Tab Nav)/.test(symbol.name)) {
        ...
        if (!/Example/.test(symbol.name)) {
          var spans = findLayers(tabHead, 'span', span => findLayers(span, l => l.resizingConstraint = TOP_LEFT_RIGHT_FIXED_HEIGHT));
          var bar = spans.shift();
      ...
      if (/^(Text Area)/.test(symbol.name)) {
        ...
        findLayer(symbol, "label.input__label", label => label.resizingConstraint = TOP_LEFT_FIXED_SIZE);
        ...
        if (helperMessage) helperMessage.resizingConstraint = BOTTOM_LEFT_RIGHT_FIXED_HEIGHT;
        if (!helperMessage && info) info.resizingConstraint = BOTTOM_LEFT_RIGHT_FIXED_HEIGHT;
        if (!helperMessage && count) count.resizingConstraint = BOTTOM_LEFT_RIGHT_FIXED_HEIGHT;
        var inputValue = findLayer(symbol, s => /^Input Value/.test(s.name));
        if (inputValue) inputValue.resizingConstraint = TOP_LEFT_FIXED_SIZE;
        findLayers(symbol, /label/i, label => label.resizingConstraint = TOP_LEFT_RIGHT_FIXED_HEIGHT);
      ...
      if (/^(Text Field)/.test(symbol.name)) {
        ...
        var helperMessage = findLayer(symbol, s => /^div#helper-message/.test(s.name));
        if (helperMessage) helperMessage.resizingConstraint = TOP_LEFT_RIGHT_FIXED_HEIGHT;
        var inputValue = findLayer(symbol, s => /^Input Value/.test(s.name));
        if (inputValue) inputValue.resizingConstraint = TOP_LEFT_FIXED_SIZE;
        findLayers(symbol, /label/i, label => label.resizingConstraint = TOP_LEFT_RIGHT_FIXED_HEIGHT);
        ...
        if (input) {
          ...
          findLayers(input, l => l.resizingConstraint = TOP_LEFT_RIGHT_FIXED_HEIGHT);
        }
