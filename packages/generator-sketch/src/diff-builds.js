...
if (!file1) {
    console.error(`File not found: ${version1} - ${documentName}.${version1}.symbols.json`);
    process.exit(1);
}
if (!file2) {
    console.error(`File not found: ${version2} - ${documentName}.${version2}.symbols.json`);
    process.exit(1);
}
...
function compare(a, b, symbolPath, keyPath, changes) {
    ...
    for (const key in a) {
        ...
        if (typeof a[key] !== typeof b[key]) {
            changes.push({symbolPath, key: keyPath+'/'+key, old: a[key], new: b[key]});
            ...
            if (key === 'name' && a[key].replace(ignoreIdNumRe, '$1') === b[key].replace(ignoreIdNumRe, '$1')) {
                continue;
            }
            changes.push({symbolPath, key: keyPath+'/'+key, old: a[key], new: b[key]});
        }
